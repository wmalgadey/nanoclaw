...
Check if `src/plugins/registry.ts` exists. If not, the plugin system hasn't been installed yet — this is unexpected since it ships with the core codebase. Stop and ask the user to check their innary needs to be baked into the container image. The build script auto-generates `plugins/binaries.json` from the plugin registry before building — do not edit that file manually.
