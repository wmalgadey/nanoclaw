...
| 23 | `approvals-instance` | `023-approvals-instance.ts` | `pending_approvals.instance` — the adapter instance a card was delivered through (NULL = default instance) |
| 24 | `container-config-tailscale-socket` | `024-container-config-tailscale-socket.ts` | `container_configs.tailscale_socket` — per-agent-group opt-in for the Tailscale host-socket passthrough (0 = off) |
